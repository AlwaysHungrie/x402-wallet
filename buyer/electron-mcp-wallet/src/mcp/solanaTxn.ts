...
import { type PaymentRequirements, WalletAdapter } from "x402-solana/types";
import { createPaymentHeaderFromTransaction } from "x402-solana/utils";

export async function keypairToWalletAdapter(keypair: Keypair): Promise<WalletAdapter> {
  return {
    publicKey: keypair.publicKey.toString(),
    // Sign a transaction (legacy)
...
export async function createSolanaPaymentHeader(
  ...
  const wallet = await keypairToWalletAdapter(keypair);
